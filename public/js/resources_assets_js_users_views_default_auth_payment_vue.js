...
/* harmony default export */ const __WEBPACK_DEFAULT_EXPORT__ = ({
  ...
  created: function created() {
    ...
    if (this.$auth.isAuthenticated() == "payment_step" || this.$auth.isAuthenticated() == "payment_reactive") {
      axios.get("/api/v1/get/check/user").then(function (response) {
        if (response.data.status != "payment_step" && response.data.status != "payment_reactive") {
