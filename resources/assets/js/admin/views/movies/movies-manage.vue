      ...
      <div class="m-2" id="manage-panel">
        <div class="float-left mt-3" v-if="showGroupButton">
          <div class="button-group">
            ...
            <button class="btn btn-sm btn-warning" @click="DELETE()">
              Delete
            </button>
          </div>
      ...
      <div v-if="!spinner_loading">
            ...
            <div class="modal-dialog" role="document">
              <div class="modal-content">
                <div class="modal-header">
                  <h5 class="modal-title" id="GetSubtitleModalLabel">
                    Subtitles
                  </h5>
                  <button
                    type="button"
                ...
                <div class="modal-body">
                  ...
                  <div class="col-12">
                    <input
                      ...
                      size="10"
                      @change="
                        SHOW_FILES_INFO('subtitle', 'subtitleFileDetails')
                      "
                      class="inputfile"
                    />
                  ...
                  <div class="subtitle-progress mt-1 p-4" v-if="subtitle_video">
                    ...
                    <div class="progress">
                      <div
                        ...
                        aria-valuemax="100"
                      ></div>
                      <div v-else><i id="btn-progress"></i> Prepare</div>
                    </div>
                  ...
                  >
                    <div class="table table-hover">
                      ...
                      <tbody>
                        <tr
                          v-for="(item, index) in subtitles.subtitles"
                          :key="index"
                        >
                          <td>{{ item.name }}</td>
                          <td>{{ item.language }}</td>
        ...
        <div v-else>
          <div
            class="mt-2"
            v-if="
              Object.keys(data_search).length > 0 && data_search.movies !== null
            "
          >
            <div class="table table-hover">
              <thead>
              ...
              <tbody>
                <tr v-for="(item, index) in data_search.movies" :key="index">
                  ...
                  <td v-if="item.show === 2">Progress</td>
                  <td v-if="item.show === 3">Encoding error</td>
                  <td>{{ item.m_category }}</td>
                  <td v-if="item.m_users_only">Users</td>
                  <td v-else>All</td>
                  <td>{{ item.created_at }}</td>
                  ...
                  <td>
                    <div class="btn-group" role="group">
                      ...
                      <router-link
                        class="btn btn-sm btn-warning btn-sm mr-2"
                        :to="{ name: 'movie_edit', params: { id: item.id } }"
                        role="button"
                        >Edit
                      </router-link>
                      <router-link
                        class="btn btn-sm btn-warning btn-sm mr-2"
                        role="buttton"
                        :to="{ name: 'movie-ads', params: { id: item.id } }"
                        >Ads</router-link
                      >
                    </div>
                  </td>
          ...
          >
            <ul class="pagination">
              <li class="page-item" id="end">
                <a
                  class="page-link"
                  @click="PAGINATION('/api/admin/get/movies')"
                  >Begin</a
                >
              </li>
              <li
                class="page-item"
                id="prev"
                :class="{ disabled: data.movies.prev_page_url === null }"
              >
                <a
                  class="page-link"
                  @click="PAGINATION(data.movies.prev_page_url)"
                  >Previous</a
                >
              </li>
              <li class="page-item">
                <a class="page-link"
                  >{{ data.movies.current_page }}/{{ data.movies.last_page }}</a
                >
              </li>
              <li
                class="page-item"
                id="next"
                :class="{ disabled: data.movies.next_page_url === null }"
              >
                <a
                  class="page-link"
                  @click="PAGINATION(data.movies.next_page_url)"
                  >Next</a
                >
              </li>
              <li class="page-item" id="end">
                <a
                  ...
                  @click="
                    PAGINATION(
                      '/api/admin/get/movies?page=' + data.movies.last_page
                    )
                  "
                  >End</a
                >
...
export default {
  ...
  methods: {
    ...
    USERS_ONLY(vai) {
      ...
      axios
        .post("/api/admin/update/movie/users-only", {
          list: this.multi_select,
          users_only: vai,
        })
        .then(
          (response) => {
            if (response.status === 200) {
              for (let i = 0; i < response.data.list.length; i++) {
                this.data.movies.data[response.data.list[i].key].m_users_only =
                  vai;
              }
              this.multi_select = [];
            }
