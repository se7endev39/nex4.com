        ...
        <div class="col-12 col-md-8 col-lg-8 offset-md-2 p-4 payment-form">
          ...
          <transition name="fade">
            <div class="plan-form payment-plan-form" v-if="show_plan">
              ...
              <div class="col-lg-12 text-center">
                <div class="row m-2">
                  <div
                    class="col-12 col-sm-6 col mt-3 text-center"
                    v-for="(item, index) in planList"
                    :key="index"
                    @click="plan = item.plan_id"
                  >
                    <div
                      class="card-plan"
                      :class="{ active_plan: plan === item.plan_id }"
                    >
                      ...
                      <i v-if="item.plan_trial !== null"
                        >{{ item.plan_trial }} {{ $t("register.day_free") }}</i
                      >
                    </div>
                  </div>
                </div>
          ...
          <div class="col-lg-10 offset-lg-1">
            ...
            <div class="col-12 col-xl-6 offset-xl-3 mt-5">
              <button
                v-show="!button_loading && !button_disabled"
                class="btn btn-warning mt-4 pay-with-stripe"
                ref="submit"
              >
                {{ $t("register.start_membership") }}
              </button>
    ...
    <div v-if="finish">
      <div class="col-12 payment">
        <div class="col-12 col-md-8 col-lg-8 offset-md-2 p-4 payment-form">
          ...
          <h5>
            {{ $t("register.your") }} {{ $t("app_name") }}
            {{ $t("register.message_one") }}
          </h5>
          <p>
            {{ $t("register.cancel_before") }} {{ trail_end }}
            {{ $t("register.message_two") }}.
          </p>
          <br />
          ...
          <ul>
            ...
            <li>
              {{ $t("register.payment_info") }}: ***********{{ card_number }}
            </li>
          </ul>
          <div class="col-12 col-xl-6 offset-xl-3 mt-5">
            <button class="btn btn-warning" @click="GO_HOME">
              {{ $t("register.finish") }}
            </button>
          </div>
        </div>
...
export default {
  ...
  created() {
    ...
    // Check Users Status
    if (
      this.$auth.isAuthenticated() == "payment_step" ||
      this.$auth.isAuthenticated() == "payment_reactive"
    ) {
      axios
        .get("/api/v1/get/check/user")
        .then((response) => {
          if (
            response.data.status != "payment_step" &&
            response.data.status != "payment_reactive"
          ) {
  ...
  methods: {
    PAY(token, cardType, lastFour, type) {
      axios
